...
from .models import Actualite, Communaute, Activite, Photo
from .forms import DonForm, MessageContactForm 
...
def don(request):
    if request.method == "POST":
        ...
        form = DonForm()
    return render(request, "site_dlo3/don.html", {"page_active": "don", "form": form})


def actualites(request):
    liste = Actualite.objects.all()
    actualites_data = [
        {"id": a.pk, "date": a.date_publication.strftime("%d %B %Y"), "titre": a.titre,
         "extrait": a.extrait, "contenu": a.contenu}
        for a in liste
    ]
    contexte = {
