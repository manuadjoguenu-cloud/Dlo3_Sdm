...
class Don(models.Model):
    """Déclaration d'un don (le paiement se fait par Mobile Money, hors du site pour l'instant)."""

    class MoyenPaiement(models.TextChoices):
        YAS = "Yas", "Yas (Mixx by Yas)"
        FLOOZ = "Flooz", "Flooz (T-Money)"
    ...
    paroisse = models.ForeignKey(Communaute, on_delete=models.SET_NULL, null=True, blank=True,
                                  related_name="dons", help_text="Facultatif")
    ...
    def __str__(self):
        return f"{self.nom} — {self.montant} FCFA ({self.date_envoi:%d/%m/%Y})"
...
class Actualite(models.Model):
    """Un article publié sur la page Actualités."""

    date_publication = models.DateField()
    titre = models.CharField(max_length=200)
    extrait = models.CharField(max_length=300, help_text="Court résumé visible dans la liste")
    ...
    def __str__(self):
        return f"{self.titre} ({self.date_publication:%d/%m/%Y})"
